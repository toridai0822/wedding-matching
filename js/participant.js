...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            // 既存の参加者チェック
            const checkResponse = await fetch(`tables/participants?search=${formData.reception_number}`);
            const checkData = await checkResponse.json();
            
            const existingParticipant = checkData.data.find(p => 
                p.reception_number === formData.reception_number
            );
